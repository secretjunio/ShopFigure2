...
using ShopMohinh.Models.ManageViewModels;
...
namespace ShopMohinh.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ChangePassword(ChangePasswordViewModel model) // async Task giúp đa luồng
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            if (user != null)
            {
                ...
                AddErrors(result); // nếu xảy ra lỗi
                return View(model);
            }
            return RedirectToAction(nameof(Index), new { Message = ManageMessageId.Error });
        }
        ...
        [HttpPost]
        public async Task<ActionResult> UpdateInfo(CustomerInfoModel model)
        {
            var user = await GetCurrentUserAsync();
